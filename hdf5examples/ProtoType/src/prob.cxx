...
void NeutrinoModel::zero(){
        //std::cout << "NeutrinoModel::zero()" << std::endl;
	for(int i = 0; i < 3; i ++){
		mNu[i] = 0; Ue[i] = 0;
		Um[i] = 0;  phi[i] = 0;
	}
...
void NeutrinoModel::difference(){
        //std::cout << "NeutrinoModel::difference()" << std::endl;
	dm41Sq = pow(mNu[0],2);
	dm51Sq = pow(mNu[1],2);
	dm61Sq = pow(mNu[2],2);
	dm54Sq = dm51Sq - dm41Sq;
...
{
        //std::cout << "NeutrinoModel::oscProbSin()" << std::endl;
...
{
        //std::cout << "NeutrinoModel::oscProbSinSq()" << std::endl;
	if(dm41Sq==0 && dm51Sq==0 &&dm61Sq==0)
	{ 
		return 1.0;
	} 
...
double NeutrinoModel::oscProb(int a, int b, double Ev, double L){
        ...
        //std::cout << "NeutrinoModel::oscProb()" << std::endl;
	if(a == b)
	{
...
double NeutrinoModel::oscProb_app(int a, int b, double Ev, double L){
        ...
        //std::cout << "NeutrinoModel::oscProb_app()" << std::endl;
...
double NeutrinoModel::oscProb_dis(int a, double Ev, double L){
        //std::cout << "NeutrinoModel::oscProb_dis()" << std::endl;
	double ans =0.0;
...
double NeutrinoModel::oscAmp(int a, int b, int which_dm, int sqornot, bool both){


        //std::cout << "NeutrinoModel::oscAmp(int a, int b, int which_dm, int sqornot)" << std::endl;
	if(a == b)
	{
		if(both) return oscAmp_dis(a, which_dm);
		else return Um[0];
	} 
	else
	{
...
double NeutrinoModel::oscAmp_app(int a, int b, int which_dm, int sqornot)
{
        ...
        //std::cout << "NeutrinoModel::oscAmp_app(int a, int b, int which_dm, int sqornot)" << std::endl;
...
double NeutrinoModel::oscAmp_dis(int a, int which_dm){
        ...
        //std::cout << "NeutrinoModel::oscAmp_dis(int a, int which_dm)" << std::endl;
	...
	switch(a)
	{
		...
		case 2:
		case -2:
			//std::cout << "oscAmp_dis: " << a << std::endl;
			Ua4 = Um[0];
			Ua5 = Um[1];
			Ua6 = Um[2];
			break;
	...
	switch(which_dm)
	{
		case 41:
			...
			//std::cout << "Ua4, Ua5, Ua6: " << Ua4 << ", " << Ua5 << ", " << Ua6 << std::endl;
			ans = -4.0*(1.0-Ua4*Ua4-Ua5*Ua5-Ua6*Ua6)*Ua4*Ua4;
			//std::cout << "-4.0*(1.0-"<<Ua4<<"*"<<Ua4<<"-"<<Ua5<<"*"<<Ua5<<"-"<<Ua6<<"*"<<Ua6<<")*"<<Ua4<<"*"<<Ua4<<" = " << ans << std::endl;
			break;
		case 51:
...
double oscProb(int a, int b, double Ev, double L,  std::vector< std::vector < std::complex<double> > >    U, std::vector<std::vector<double> > dm ){
        ...
        std::cout << " oscProb(int a, int b, double Ev, double L,  std::vector< std::vector < std::complex<double> > >    U, std::vector<std::vector<double> > dm )" << std::endl;
...
{  
        std::cout << "Pmue(double L, double E, double dm, double sin2)" << std::endl;
...
double Pmm(double L, double Ev, double Dm, double sinSq2thmm){
        ...
        std::cout << "Pmm(double L, double Ev, double Dm, double sinSq2thmm)" << std::endl;
