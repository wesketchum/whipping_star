...
for(size_t i=0; i< tin->GetEntries(); ++i){
    tin->GetEntry(i);
    ...
    //and fill the new tree. Because its being filled once for every event in the input TTree in unison, we know the entries match up and thus it be correctly assigned as a FriendTree.
    tout->Fill();
}
tout->Write();
fout->Close();
