...
//Much courtesy to wirecell team
...
std::vector<double> LEEana::wireWrapperBayes(std::vector<double> &means, std::vector<double>&sigma2s, std::vector<double>& pots){
    
    Bayes *wirebayes = new Bayes();
    for(int i=0; i<means.size();i++){
        wirebayes->add_meas_component(means[i],sigma2s[i],pots[i]);
    }
   
    wirebayes->do_convolution();

    auto ans = wirebayes->calculate_lower_upper(1);
    double mean_conv = wirebayes->get_f_conv()->GetMaximumX(); 
    double mean = wirebayes->get_mean();
    double cov = wirebayes->get_covariance();
    //double cov_mc = wirebayes->get_covariance_mc();

   /* std::cout<<"mean and meancov "<<mean_conv<<" <--> "<<mean<<std::endl;
    std::cout<<"1 Sigma Range: "<<mean_conv<<" - "<<mean_conv-ans.first<<"  + "<<ans.second- mean_conv<<std::endl;
    std::cout<<"AvgErr = "<<(ans.second-ans.first)/2.0<<std::endl;
   */

    std::vector<double> rans = {mean, mean_conv, ans.first, ans.second, cov};
...
/*void bayes(){
    ...
    Bayes *exampleA = new Bayes();
    
    // measurement: event and weight
    // so GetBinContent()        and       GetBinError()  
    double POT_scale = 0.05;
    double mod = 2.0;
    exampleA->add_meas_component(40.0*0.025*mod,  mod*40*0.025*0.025, POT_scale);
    exampleA->add_meas_component(20.0*0.1*mod,    mod*20*0.1*0.1,     POT_scale);
    exampleA->add_meas_component(30.0*0.05*mod,   mod*30*0.05*0.05,   POT_scale);
    exampleA->add_meas_component(0, 0.0,  POT_scale);

    /// print out inputs
    exampleA->do_convolution();

    auto ans = exampleA->calculate_lower_upper(1);
    std::cout<<"test "<<ans.first<<" "<<ans.second<<std::endl;

    TCanvas *canv_data = new TCanvas("halp","halp", 900, 650);

    exampleA->get_f_conv()->Draw();    
    exampleA->get_f_conv()->SetLineColor(kGray+1);
    exampleA->get_f_conv()->GetHistogram()->SetMaximum( exampleA->get_f_conv()->GetHistogram()->GetMaximum()*2); 
    exampleA->get_f_conv()->GetXaxis()->SetRangeUser(0,ans.second*2); 

    for(int idx=0; idx<exampleA->get_num_components(); idx++) {
        exampleA->get_f(idx)->Draw("same");
        exampleA->get_f(idx)->SetLineColor(kRed);
    }

    double mean_conv = exampleA->get_f_conv()->GetMaximumX(); 
    double mean = exampleA->get_mean();
    std::cout<<"mean and meancov "<<mean_conv<<" <--> "<<mean<<std::endl;
    std::cout<<"1 Sigma Range: "<<mean_conv<<" - "<<mean_conv-ans.first<<"  + "<<ans.second- mean_conv<<std::endl;
    std::cout<<"AvgErr = "<<(ans.second-ans.first)/2.0<<std::endl;
    double cov = exampleA->get_covariance();
    double cov_mc = exampleA->get_covariance_mc();
    std::cout<<"Ana? "<<cov<<" sqrt() "<<sqrt(cov)<<std::endl;
    std::cout<<"MC? "<<cov_mc<<" sqrt() "<<sqrt(cov_mc)<<std::endl;

    TLine *lmean_conv = new TLine(mean_conv,0,mean_conv, 0.2);
    lmean_conv->Draw();
    lmean_conv->SetLineStyle(2);

    TLine *lmean = new TLine(mean,0,mean, 0.2);
    lmean->Draw();
    ...
    auto rans = wireWrapperBayes(v_means,v_sigma2s,v_pots);
